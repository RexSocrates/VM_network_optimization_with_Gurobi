...
def solveModel(timeLength, fixedVarDict, optimizedVarDict, relaxedVarDict) :
    ...
    try :
                    ...
                    for vmIndex in range(0, len(vmTypeList)) :
                        ...
                        for contractIndex in range(0, len(vmContractLengthList)) :
                            ...
                            for paymentOptionIndex in range(len(vmPaymentList)) :
                                ...
                                # declare reservation and utilization decision variables
                                reservationVar = None
                                utilizationVar = None
                                ...
                                # configure the variable of VM reservation
                                if resDecVarName in fixedVarDict :
                                    reservationVar = fixedVarDict[resDecVarName]
                                    ...
                                    print('VM reservation var set error')
                                
                                # configure the variable of VM utilization
                                if utiDecVarName in fixedVarDict :
                                    utilizationVar = fixedVarDict[utiDecVarName]
                                    ...
                                    print('VM utilization var set error')
                        ...
                        # configure the variable of on-demand VM
                        if onDemandDecVarName in fixedVarDict :
                            onDemandVmVar = fixedVarDict[onDemandDecVarName]
        ...
        for timeStage in range(0, timeLength) :
            ...
            for area in routerAreaDict :
                ...
                for router in areaRouterList :
                    ...
                    routerStatus = None
                    routerStatusDercVarName = 'RS_' + timeRouterStr
                    # configure the variable of router status
                    if routerStatusDercVarName in fixedVarDict :
                        routerStatus = fixedVarDict[routerStatusDercVarName]
                    elif routerStatusDercVarName in optimizedVarDict :
                        routerStatus = model.addVar(vtype=GRB.BINARY, name=routerStatusDercVarName)
                    elif routerStatusDercVarName in relaxedVarDict :
                        routerStatus = model.addVar(vtype=GRB.CONTINUOUS, name=routerStatusDercVarName)
                    else :
                        print('Router status set error')
        ...
        for v in model.getVars() :
            ...
            if varName in relaxedVarDict :
                varValue = math.ceil(varValue)
